...
**The `set` dimension is an honest trade, not a clean win.** Its tail improves sharply — p90 6.00 →
2.00, max 18x → 6x — while its median moves further BELOW truth, 0.900 → 0.586. So `set:X t:Y` now
under-predicts more often than it did. Undershoots are the more dangerous direction in principle, and
the reason to accept it here is that the routing-relevant count still fell (3 → 1 overall) and
nothing in the survey or the plan-cost guards moved. Worth re-checking if a future survey ever samples
this population properly.
