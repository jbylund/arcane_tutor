...
//
// 2026082501 — new `CardIndexes` field `edhrec_walk_checkpoints` (`WalkCheckpoints` for subtypes/
// keywords/oracle_tags against the EDHREC permutation). Same blind spot again. Rebased onto the
// `layout` bump above (which already claimed 2026082302 by the time this landed), so this takes the
// next free date rather than reusing that commit's original 2026082302.
