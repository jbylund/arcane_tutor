#!/usr/bin/env python3
"""Download and subset the Beleren font to include only Latin characters for card text.

This script:
1. Downloads the Beleren Bold font from the npm package
...
# Beleren font information
BELEREN_FONT_NPM_PACKAGE = "@saeris/typeface-beleren-bold"
BELEREN_FONT_VERSION = "1.0.1"
BELEREN_FONT_BASE_URL = f"https://unpkg.com/{BELEREN_FONT_NPM_PACKAGE}@{BELEREN_FONT_VERSION}"
...
def generate_css(output_dir: Path, cdn_base_url: str = "/cdn/fonts/beleren") -> None:
 ...
 * Version: {BELEREN_FONT_VERSION}
 * License: MIT
 */

@font-face {{
  font-family: 'Beleren';
  src: url('{cdn_base_url}/beleren-subset.woff2') format('woff2'),
       url('{cdn_base_url}/beleren-subset.woff') format('woff');
  font-weight: bold;
  font-style: normal;
  font-display: swap;
}}
"""
    ...
    css_file = output_dir / "beleren-subset.css"
...
def upload_fonts_to_cloudfront(
    output_dir: Path,
    bucket: str,
    s3_prefix: str = "cdn/fonts/beleren",
) -> None:
    ...
    files_to_upload = [
        ("beleren-subset.woff2", "font/woff2"),
        ("beleren-subset.woff", "font/woff"),
        ("beleren-subset.css", "text/css; charset=utf-8"),
    ]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Subset Beleren font for optimized web delivery",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/fonts/beleren"),
        help="Output directory for subsetted fonts (default: data/fonts/beleren)",
    )
    parser.add_argument(
        "--cdn-url",
        type=str,
        default="https://d1hot9ps2xugbc.cloudfront.net/cdn/fonts/beleren",
    ...
    parser.add_argument(
        "--s3-prefix",
        type=str,
        default="cdn/fonts/beleren",
        help="S3 key prefix (default: cdn/fonts/beleren)",
    )
    parser.add_argument(
        "--skip-upload",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-download",
        action="store_true",
        help="Skip downloading fonts (use existing files in output dir)",
    )
    ...
    print("=" * 60)
    print("Beleren Font Subsetting Tool")
    print("=" * 60)

    # Download original fonts if needed
    if not args.skip_download:
        woff_url = f"{BELEREN_FONT_BASE_URL}/Beleren2016-Bold.woff"
        woff_orig = args.output_dir / "beleren-original.woff"
        ...
        woff_orig = args.output_dir / "beleren-original.woff"
        if not woff_orig.exists():
            print(f"Error: {woff_orig} not found. Remove --skip-download to download it.")
            sys.exit(1)

    # Subset to woff2
    woff2_subset = args.output_dir / "beleren-subset.woff2"
    subset_font(woff_orig, woff2_subset, "woff2")

    # Subset to woff
    woff_subset = args.output_dir / "beleren-subset.woff"
    ...
    print("  - beleren-subset.woff2 (recommended)")
    print("  - beleren-subset.woff (fallback)")
    print("  - beleren-subset.css")
